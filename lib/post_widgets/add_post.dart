...
class _AddPostState extends State<AddPost> {
  ...
  Future<void> addScrapbookPostMarker()
  async {
    //-------------------------POST SECTION STARTS--------------------------------------
    ...
    AddPost.postRef = await posts.add({
      ...
      'picture': '', //todo get the download url from Storage and put it here IF user uploaded video,
      ...
      'user': userDocRef,
      'video': '' //todo get the download url from Storage and put it here IF user uploaded video,
      //todo IF NULL, FILL IT WITH "images/default_image.png"
    });
    ...
    await FirebaseFirestore
        ...
        .collection("users")
        .where("uuid",
        isEqualTo: ProfilePage().getUuid() as String)
        .get();
    List<QueryDocumentSnapshot<Map<String, dynamic>>> docList3 =
        snap3.docs;
    ...
    AddPost.scrapbookRef = await  db.collection('scrapbooks').add({
      ...
      'location' : GeoPoint(NewScrapbookPage.currentLat!, NewScrapbookPage.currentLong!),
      'public' : NewScrapbookPage.visibility,
      'scrapbookThumbnail' : '', //todo get the download url from Storage and put it here, IF NULL, FILL IT WITH "images/default_image.png"
      'scrapbookTitle' : ScrapbookTitle.scrapbookTitle,
      'timestamp' : Timestamp.now()
    });
    print("Scrapbook added!" + "with scrapbookRef: " + AddPost.scrapbookRef.toString());
    ...
    await db.collection("markers").add({
      'location': GeoPoint(NewScrapbookPage.currentLat!,
    ...
    await db.collection('scrapbookPosts').add({
      'post' : AddPost.postRef,
      'scrapbook' : AddPost.scrapbookRef
    }).then((documentSnapshot) =>
        print("Scrapbook and Post linked!"));
